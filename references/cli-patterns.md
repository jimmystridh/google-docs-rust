...
drive_manager search --query "name contains 'Report'"

# Download file - straightforward parameters
drive_manager download --file-id "abc123xyz" --output "./download"

# Share file - few clear parameters
drive_manager share --file-id "abc123xyz" --email "user@example.com" --role "reader"
```
...
For complex operations, compose multiple flag-based commands in shell scripts:

```bash
# Example: share then fetch metadata
drive_manager share --file-id "ID" --email "user@example.com" --role "writer"
drive_manager get-metadata --file-id "ID"
```
...
google-sheets currently standardizes on JSON stdin for all operations:

```bash
# Simple read
echo '{"spreadsheet_id":"ID","range":"Sheet1!A1"}' | sheets_manager read

# Complex operations use JSON (no flag equivalent)
echo '{...}' | sheets_manager batch-write
```
...
docs_manager read <document_id>

# Complex operations: JSON via stdin
echo '{...}' | docs_manager insert
```
...
docs_manager read "abc123xyz"

# Get structure - one parameter
docs_manager structure "abc123xyz"
```
...
```bash
drive_manager get-metadata --file-id "abc123xyz"
```
**Why**: File metadata is flat structure, few parameters.

**google-sheets** (JSON):
```bash
echo '{"spreadsheet_id":"ID","range":"Sheet1!A1:C10"}' | sheets_manager read
```
...
**google-drive** (flags):
```bash
drive_manager update --file-id "ID" --name "New Name" --description "Updated"
```
...
```bash
drive_manager search --query "type:document"
drive_manager download --file-id "ID" --output "./file"
```

❌ **DON'T**: Try to force JSON when flags are sufficient
```bash
# Unsupported usage pattern
echo '{"file_id":"ID"}' | drive_manager get-metadata
```
...
drive_manager download --file-id "$FILE_ID" --output "./download"
```
...
echo '{"spreadsheet_id":"ID","range":"Sheet1!A1:B10","values":[[1,2],[3,4]]}' | sheets_manager write
```

💡 **TIP**: Use heredoc for complex operations
```bash
sheets_manager write <<'EOF'
{
  "spreadsheet_id": "ID",
  "range": "Sheet1!A1:C3",
  "values": [
...
```bash
docs_manager read "document_id"
docs_manager structure "document_id"
```
...
STRUCTURE=$(docs_manager structure "ID")
...
### google-drive: Add Optional Permission Templates

Not implemented in the current project. Use repeated `drive_manager share` calls for each permission target.

### google-sheets: Add Optional Flag Shortcuts

```bash
# Current: JSON for all operations
echo '{"spreadsheet_id":"ID","range":"A1"}' | sheets_manager read

# JSON is still required for complex operations
echo '{...complex batch operation...}' | sheets_manager batch-write
```
