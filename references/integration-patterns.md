...
scripts/drive_manager create-folder \
  --name "Project Alpha" \
...
for DOC_ID in $(jq -r '.files[].id' search_results.json); do
  ...
  CONTENT=$(scripts/docs_manager read "$DOC_ID")
...
for SHEET_ID in $(jq -r '.files[].id' search_results.json); do
  ...
  }" | scripts/sheets_manager read >> consolidated_data.json
done
...
**Solution**:
```bash
# Check current file metadata (includes permissions array when available)
...
2. **Use Batch Operations**: google-sheets `batch-write` for multiple changes
3. **Error Handling**: Always check JSON response for `"status": "success"` before proceeding
